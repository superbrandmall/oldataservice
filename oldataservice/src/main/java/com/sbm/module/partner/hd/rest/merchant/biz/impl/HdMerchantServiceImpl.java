package com.sbm.module.partner.hd.rest.merchant.biz.impl;
 ...
 */
@Component
public class HdMerchantServiceImpl extends HdInteractiveServiceImpl implements IHdMerchantService {

	@Autowired
	@Qualifier("hdRestTemplate")
	private RestTemplate restTemplate;

	private static final String QUERY_METHOD = "/rest/onlineleasing/tenant/query";
	private static final String SAVE_METHOD = "/rest/onlineleasing/tenant";
	...
	public HdResult<HdResultBody<HdMerchant>> query(QueryFilter queryFilter) {
		...
		ResponseEntity<HdResult<HdResultBody<HdMerchant>>> result = restTemplate.exchange(uri, HttpMethod.POST, entity,
				new ParameterizedTypeReference<HdResult<HdResultBody<HdMerchant>>>() {
				});
	...
	public HdResult<HdMerchant> save(HdMerchant obj) {
		...
		HttpEntity<String> entity = getHttpHeader(request);
		ResponseEntity<HdResult<HdMerchant>> result = restTemplate.exchange(uri, HttpMethod.POST, entity,
				new ParameterizedTypeReference<HdResult<HdMerchant>>() {
				});
		CommonConstant.FRAMEWORK.info("vo: " + JSON.toJSONString(result));
		return result.getBody();
	}
